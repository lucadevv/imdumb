...
class _TopRatedMoviesSectionState extends State<TopRatedMoviesSection> {
  ...
  void _onScroll() {
    if (_isBottom) {
      ...
      if (state.hasMoreTopRatedMovies &&
          state.topRatedMovieState != PopularMovieState.loading) {
        context.read<HomeBloc>().add(
          FetchAllTopRatedMovieEvent(
          ...
          child: Column(
            ...
            children: [
              ...
              if (state.topRatedMovieState == PopularMovieState.loading &&
                  state.listTopRatedMovie.isEmpty)
                const _TopRatedShimmerList()
              else if (state.topRatedMovieState == PopularMovieState.failure &&
                  state.listTopRatedMovie.isEmpty)
                Padding(
              ...
              else if (state.listTopRatedMovie.isEmpty)
                const Padding(
                ...
                SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemCount:
                        state.listTopRatedMovie.length +
                        (state.topRatedMovieState == PopularMovieState.loading
                            ? 1
                            : 0),
                    itemBuilder: (context, index) {
                      if (index >= state.listTopRatedMovie.length) {
                      ...
                      final movie = state.listTopRatedMovie[index];
                      return _TopRatedMovieCard(movie: movie);
                    },
                  ),
                ),
            ],
...
class _TopRatedMovieCard extends StatelessWidget {
  ...
  const _TopRatedMovieCard({required this.movie});
...
class _TopRatedShimmerList extends StatelessWidget {
  const _TopRatedShimmerList();
